...
import { getVietnameseTableStatus, handleErrorApi } from '@/app/lib/utils'
...
export default function AddTable() {
  ...
  const onSubmit = async (value: CreateTableBodyType) => {
    if (addTableMutation.isPending) return
    try {
      let body = {...value, number: Number(value.number) }
                  ...
                  <FormItem>
                    <div className='grid grid-cols-4 items-center justify-items-start gap-4'>
                      ...
                      <div className='col-span-3 w-full space-y-2'>
                        <Input id='number' type='number' className='w-full' {...field} onChange={(e) => form.setValue("number", Number(e.target.value))} />
                        <FormMessage />
                      </div>
                    </div>
                  </FormItem>
                ...
                render={({ field }) => (
                  <FormItem>
                    <div className='grid grid-cols-4 items-center justify-items-start gap-4'>
                      ...
                      <div className='col-span-3 w-full space-y-2'>
                        <Input id='capacity' className='w-full' {...field} type='number' onChange={(e) => form.setValue("capacity", Number(e.target.value))} />
                        <FormMessage />
                      </div>
                    </div>
                  </FormItem>
