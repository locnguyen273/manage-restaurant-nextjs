...
import { getTableLink, getVietnameseTableStatus, handleErrorApi } from '@/app/lib/utils'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select'
import { UpdateTableBody, UpdateTableBodyType } from '@/schemaValidations/table.schema'
import { TableStatus, TableStatusValues } from '@/constants/type'
import { Switch } from '@/components/ui/switch'
import Link from 'next/link'
import { useGetTableMutation, useUpdateTableMutation } from '@/queries/useTable'
...
export default function EditTable({
  ...
  const { data } = useGetTableMutation({
    id: id as number,
    enabled: Boolean(id)
  })
  ...
  useEffect(() => {
    if (data) {
      const { capacity, number, status, token } = data.payload.data
      form.reset({
        capacity,
        status,
  ...
  const onSubmit = async (value: UpdateTableBodyType) => {
    ...
    try {
      let body: UpdateTableBodyType & { id: number } = { id: id as number, ...value }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Form {...form}>
          <form noValidate className='grid auto-rows-max items-start gap-4 md:gap-8' id='edit-table-form' onSubmit={form.handleSubmit(onSubmit)}>
