...
import { formatCurrency, getVietnameseDishStatus, handleErrorApi } from '@/app/lib/utils'
  ...
  const deleteDish = async () => {
    if (dishDelete) {
      try {
        ...
        setDishDelete(null)
        toast.success((result as { payload: { message: string } }).payload.message)
      } catch (error) {
...
export default function DishTable() {
  ...
  const resultDishList = dishListQuery.data?.payload
  const data = resultDishList?.data ?? []
