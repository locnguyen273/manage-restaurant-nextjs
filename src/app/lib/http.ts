...
import { normalizePath, removeTokenFromLocalStorage, setAccessTokenToLocalStorage, setRefreshTokenToLocalStorage } from '@/app/lib/utils'
...
let clientLogout: null | Promise<any> = null
...
const request = async <Response>(
  ...
  if (!res.ok) {
    if (res.status === ENTITY_ERROR_STATUS) {
      ...
      if (isClient) {
        if (!clientLogout) {
          clientLogout = fetch('/api/auth/logout', {
            method: 'POST',
